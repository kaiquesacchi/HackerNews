...
export default function LinkList() {
  ...
  const { data, loading, error, subscribeToMore } = useQuery<any>(FEED_QUERY, {
  ...
  const getLinksToRender = (isNewPage: boolean, data: iFeedQuery) => {
    if (isNewPage) {
          ...
          {isNewPage && (
            <div className="flex ml4 mv3 gray">
              ...
              <div
                className="pointer"
                onClick={() => {
                  if (page <= data.feed.links.length / LINKS_PER_PAGE) {
